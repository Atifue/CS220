...
# [1] Define a function get_function_type(func, func_domain, func_co_domain) that receives a function "func"
# and two sets - domain and co_domain and determines whether the function passed in is "partial", "bijectve",
# "injective", or "surjective". First, compute func_range by evaluating the function for each value of the domain.
...
def get_function_type(func, func_domain, func_co_domain):
    ...
    return f"is_total, {is_Total}, is_partial {is_partial}, is_injective {is_injective}, is_bijective {is_bijective}"
...
def q1():
    ...
    for func in [f_identity, f_predecessor, f_successor, f_square]:
        print(func.__name__, get_function_type(func, func_domain=decimals, func_co_domain=decimals))


# [2] Define functions that compute the following set operations. The first five functions are binary and should accept
# two arguments each, while the last function is unary and should accept only one argument. You may use built-in
# functions to your language or implement them yourself.
...
# [c] Compare the results (since there may be roundoff error,
...
def q3():
    ...
    print("Sum sqaures: ", sum_squares_loop(n), sum_squares_formula(n))
...
def sum_arithmetic_formula(n, a, d):
    return a * (n + 1) + d * n * (n + 1) / 2  # all the a's counting numbers * d
...
# fractional binary numbers in the range [0,1) and uses Cantor's diagonalization argument to find a number not already
# in that list. Be mindful of the case where there are more elements in the list than digits in each number, and assume
# implicit trailing zeros as needed.
...
# [6] Define a  function that derives a closed-form formula for the summation s(n, p) = 1p + 2p + … + np, using the
# method demonstrated in class, i.e. derive the coefficients of a polynomial of degree p+1 that gives s(n, p)
#
...
# we are not doing 6, leave it
def q6():
    print("we are skipping question 6")
